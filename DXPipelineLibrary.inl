...
public:
	PipelineLibrarySerializer(ID3D12Device* Dev, const wchar_t* Path) : Device(Dev), FilePath(Path) {
		//!< 毎回作り直す場合 (If recreate everytime)
		DeleteFile(FilePath);
		...
		if (SUCCEEDED(D3DReadFileToBlob(FilePath, COM_PTR_PUT(Blob))) && Blob->GetBufferSize()) {
	...
	virtual ~PipelineLibrarySerializer() {
		if (!IsLoaded) {
			...
			if (Size) {
				...
				VERIFY_SUCCEEDED(D3DWriteBlobToFile(COM_PTR_GET(Blob), FilePath, TRUE));
			}
		}
	}
