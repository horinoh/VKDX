...
layout (location = 1) out vec3 OutTangentViewDirection;
layout (location = 2) out vec3 OutTangentLightDirection;
...
void main()
{
	...
	for(int i=0;i<gl_in.length();++i) {
		...
		OutTangentViewDirection = vec3(dot(V, InTangent[i]), dot(V, Binormal), dot(V, InNormal[i]));
		OutTangentLightDirection = vec3(dot(LocalLightDirection, InTangent[i]), dot(LocalLightDirection, Binormal), dot(LocalLightDirection, InNormal[i]));
