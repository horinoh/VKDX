...
#pragma endregion
	...
	virtual void PopulateCommandList(const size_t i) override {
		...
		VERIFY_SUCCEEDED(CL->Reset(CA, nullptr)); {
			...
			} PopulateEndRenderTargetCommand(CL, RT, COM_PTR_GET(SwapChain.ResourceAndHandles[i].first));
		} VERIFY_SUCCEEDED(CL->Close());
	}
	...
	virtual void CreateRootSignature() override {
		...
		constexpr std::array DRs_Tex = {
			...
			D3D12_DESCRIPTOR_RANGE1({.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV, .NumDescriptors = 2, .BaseShaderRegister = 1, .RegisterSpace = 0, .Flags = D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE,.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND })
		};
		DX::SerializeRootSignature(Blob, {
			...
			D3D12_ROOT_PARAMETER1({
				.ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE,
				.DescriptorTable = D3D12_ROOT_DESCRIPTOR_TABLE1({.NumDescriptorRanges = static_cast<UINT>(size(DRs_Tex)), .pDescriptorRanges = data(DRs_Tex) }),
				.ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL
			}),
