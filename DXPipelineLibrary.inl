...
public:
	PipelineLibrarySerializer(ID3D12Device* Dev, const std::wstring& Path) : Device(Dev), FilePath(Path) {
		...
		if (SUCCEEDED(D3DReadFileToBlob(FilePath.c_str(), COM_PTR_PUT(Blob))) && Blob->GetBufferSize()) {
			Log("Reading PipelineLibrary\n");
	...
	virtual ~PipelineLibrarySerializer() {
		if (!IsLoaded) {
			...
			if (Size) {
				...
				VERIFY_SUCCEEDED(D3DWriteBlobToFile(COM_PTR_GET(Blob), FilePath.c_str(), TRUE));
			}
		}
		LOG_OK();
	}
