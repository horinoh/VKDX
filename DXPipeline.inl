...
template<> void CreatePipelineState_Vertex<Vertex_PositionNormalTexcoord>(COM_PTR<ID3D12PipelineState>& PST, ID3D12RootSignature* RS,
	const D3D12_SHADER_BYTECODE VS, const D3D12_SHADER_BYTECODE PS, const D3D12_SHADER_BYTECODE DS, const D3D12_SHADER_BYTECODE HS, const D3D12_SHADER_BYTECODE GS,
	ID3D12PipelineLibrary* PL, LPCWSTR Name, const bool IsLoad)
{
	PERFORMANCE_COUNTER();
	...
	const std::array<D3D12_INPUT_ELEMENT_DESC, 3> IEDs = { {
		{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, offsetof(Vertex_PositionNormalTexcoord, Position), D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, offsetof(Vertex_PositionNormalTexcoord, Normal), D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "TEXCOORD0", 0, DXGI_FORMAT_R32G32_FLOAT, 0, offsetof(Vertex_PositionNormalTexcoord, Texcoord), D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
	} };
	const D3D12_INPUT_LAYOUT_DESC ILD = {
		IEDs.data(), static_cast<UINT>(IEDs.size())
	};
