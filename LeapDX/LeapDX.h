...
#pragma endregion
		DX::SerializeRootSignature(Blob, {
			D3D12_ROOT_PARAMETER1({
				...
				.DescriptorTable = D3D12_ROOT_DESCRIPTOR_TABLE1({.NumDescriptorRanges = static_cast<uint32_t>(size(DRs)), .pDescriptorRanges = data(DRs) }),
...
#ifdef USE_LEAP
			std::array<std::array<std::array<DirectX::XMFLOAT4, std::size(LEAP_DIGIT::bones)>, std::size(LEAP_HAND::digits)>, 2> Hands;
#else
