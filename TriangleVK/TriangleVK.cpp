...
void TriangleVK::CreateBottomLevel()
{
	...
	//!< BLAS
	{
		...
		//!< BLASの作成
		BLASs.emplace_back(AccelerationStructureBuffer());
	...
	//!< TLAS
	{
		...
		//!< TLASの作成
		TLASs.emplace_back(AccelerationStructureBuffer());
	...
	//!< シェーダバインディングテーブル (パイプラインより後)
	{
		...
		std::vector<std::byte> Data(AlignedSize * 3);
		VERIFY_SUCCEEDED(vkGetRayTracingShaderGroupHandlesKHR(Device, Pipelines.back(), 0, 3, size(Data), data(Data)));

		auto PDMP = GetCurrentPhysicalDeviceMemoryProperties();
		...
		ShaderBindingTables.emplace_back(BufferAndDeviceMemory());
		ShaderBindingTables.back().Create(Device, PDMP, BUF, PDRTPP.shaderGroupHandleSize, MPF, data(Data) + 0 * AlignedSize); //!< Ray Gen

		ShaderBindingTables.emplace_back(BufferAndDeviceMemory());
		ShaderBindingTables.back().Create(Device, PDMP, BUF, PDRTPP.shaderGroupHandleSize, MPF, data(Data) + 1 * AlignedSize); //!< Closest Hit

		ShaderBindingTables.emplace_back(BufferAndDeviceMemory());
		ShaderBindingTables.back().Create(Device, PDMP, BUF, PDRTPP.shaderGroupHandleSize, MPF, data(Data) + 2 * AlignedSize); //!< Ray Miss
	}
#endif
}
