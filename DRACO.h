...
public:
	virtual void GenerateNormals(draco::Mesh* Mesh) {
		...
		constexpr std::array Nrm = { 0.0f, 0.0f, 1.0f };
		for (auto i = 0; i < POSITION->size() / 3; ++i) {
			...
			//const std::array<float, 3> Nrm = { Cross[0] / Len,Cross[1] / Len, Cross[2] / Len };
			Mesh->attribute(AttrId)->SetAttributeValue(static_cast<draco::AttributeValueIndex>(i * 3 + 0), data(Nrm));
			Mesh->attribute(AttrId)->SetAttributeValue(static_cast<draco::AttributeValueIndex>(i * 3 + 1), data(Nrm));
			Mesh->attribute(AttrId)->SetAttributeValue(static_cast<draco::AttributeValueIndex>(i * 3 + 2), data(Nrm));
		}
	}
