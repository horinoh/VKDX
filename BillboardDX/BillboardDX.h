	...
	virtual void PopulateCommandList(const size_t i) override {
		...
		VERIFY_SUCCEEDED(GCL->Reset(CA, PS));
		{
			...
			ResourceBarrier(GCL, SCR, D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
			{
				...
				GCL->ClearDepthStencilView(DsvCPUHandles[0], D3D12_CLEAR_FLAG_DEPTH/*| D3D12_CLEAR_FLAG_STENCIL*/, 1.0f, 0, 0, nullptr);
				const std::array RTCDHs = { SCCDH };
				GCL->OMSetRenderTargets(static_cast<UINT>(size(RTCDHs)), data(RTCDHs), FALSE, &DsvCPUHandles[0]);

				{
					const std::array DHs = { COM_PTR_GET(CbvSrvUavDescriptorHeaps[0]) };
					GCL->SetDescriptorHeaps(static_cast<UINT>(size(DHs)), data(DHs));
#pragma region FRAME_OBJECT
					GCL->SetGraphicsRootDescriptorTable(0, CbvSrvUavGPUHandles[i]); //!< CBV
#pragma endregion
				}
