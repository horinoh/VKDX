	...
	virtual void CreateGeometry() override {
		if (HasMeshShaderSupport(COM_PTR_GET(Device))) {
			...
			VertexBuffer.Create(COM_PTR_GET(Device), TotalSizeOf(Vertices), sizeof(Vertices[0]))
				.ExecuteCopyCommand(COM_PTR_GET(Device), CA, CL, CQ, COM_PTR_GET(GraphicsFence), TotalSizeOf(Vertices), data(Vertices), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
			VertexIndexBuffer.Create(COM_PTR_GET(Device), TotalSizeOf(VertexIndices8), sizeof(Indices[0])) //!< ここではストライドは sizeof(UINT32)
				.ExecuteCopyCommand(COM_PTR_GET(Device), CA, CL, CQ, COM_PTR_GET(GraphicsFence), TotalSizeOf(VertexIndices8), data(VertexIndices8), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE); 
			MeshletBuffer.Create(COM_PTR_GET(Device), TotalSizeOf(Meshlets), sizeof(Meshlets[0]))
				.ExecuteCopyCommand(COM_PTR_GET(Device), CA, CL, CQ, COM_PTR_GET(GraphicsFence), TotalSizeOf(Meshlets), data(Meshlets), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
			TriangleBuffer.Create(COM_PTR_GET(Device), TotalSizeOf(Triangles), sizeof(Triangles[0]))
				.ExecuteCopyCommand(COM_PTR_GET(Device), CA, CL, CQ, COM_PTR_GET(GraphicsFence), TotalSizeOf(Triangles), data(Triangles), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
