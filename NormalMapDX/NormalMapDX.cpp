                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_NORMALMAPDX, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_NORMALMAPDX));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_NORMALMAPDX));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_NORMALMAPDX);
...
#pragma region Code
#ifdef USE_WINRT
void NormalMapDX::SerializeRootSignature(winrt::com_ptr<ID3DBlob>& RSBlob)
#elif defined(USE_WRL)
void NormalMapDX::SerializeRootSignature(Microsoft::WRL::ComPtr<ID3DBlob>& RSBlob)
#endif
{
	const std::array<D3D12_DESCRIPTOR_RANGE, 1> DescriptorRanges_CBV = {
		{
	...
	const std::array<D3D12_DESCRIPTOR_RANGE, 1> DescriptorRanges_SRV = {
		{
	...
	const std::vector<D3D12_ROOT_PARAMETER> RootParameters = {
		{
			...
			{ 
				static_cast<UINT>(DescriptorRanges_CBV.size()), DescriptorRanges_CBV.data()
			},
		...
		},
		{
			D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE,
			{
				static_cast<UINT>(DescriptorRanges_SRV.size()), DescriptorRanges_SRV.data()
			},
